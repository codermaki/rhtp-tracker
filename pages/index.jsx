...
const POLL_INTERVAL_RUNNING = 3000;   // 3s while update running
...
export default function RHTPTracker() {
  ...
  const [updateStatus, setUpdateStatus] = useState({ running: false, progress: 0, total: 51 });
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setUpdateStatus(json.updateStatus || { running: false, progress: 0, total: 51 });
  ...
  // Polling — faster when an update is running
  useEffect(() => {
    fetchData();
    const interval = updateStatus.running ? POLL_INTERVAL_RUNNING : POLL_INTERVAL_IDLE;
    pollRef.current = setInterval(fetchData, interval);
    return () => clearInterval(pollRef.current);
  }, [fetchData, updateStatus.running]);
  ...
  const triggerManualUpdate = async () => {
    ...
    try {
      ...
      await fetchData(); // immediately refresh status
    } catch (e) {
  ...
  const pct = updateStatus.total > 0 ? Math.round((updateStatus.progress / updateStatus.total) * 100) : 0;
  ...
  const estMinutesLeft = updateStatus.running
    ? Math.ceil(((updateStatus.total - updateStatus.progress) * 0.7) / 60)
    : null;
    ...
    <div style={{ fontFamily:"'DM Mono','Courier New',monospace", background:"#080b14", minHeight:"100vh", color:"#c8d0e8" }}>
      ...
      <div style={{ background:"#0a0d1a", borderBottom:"1px solid #1e2540", padding:"20px 28px" }}>
        ...
        <div style={{ display:"flex", gap:20, marginTop:16, flexWrap:"wrap" }}>
          {[
            ...
            { label:"Nightly Update",  value:"2:00 AM",      total:null,  color:"#a78bfa" },
          ].map(stat => (
          ...
          <div style={{ marginTop:14 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
              <div style={{ fontSize:10, color:"#5baaff", letterSpacing:".1em" }}>
                ...
                {updateStatus.trigger === "nightly" ? "NIGHTLY UPDATE" : "MANUAL UPDATE"} —&nbsp;
                {updateStatus.currentState || "Working…"}
              </div>
              <div style={{ display:"flex", gap:16, fontSize:10, color:"#3a4560" }}>
                <span>{updateStatus.progress}/{updateStatus.total} sources</span>
                {estMinutesLeft !== null && <span>~{estMinutesLeft} min left</span>}
              </div>
            </div>
            <div className="pb"><div className="pf" style={{ width:`${pct}%` }}/></div>
            <div style={{ fontSize:9, color:"#2a3050", marginTop:4 }}>
              Each of the 50 state DOH websites + CMS/HHS are queried with live web search. This takes 5–8 minutes.
            </div>
          </div>
        )}

        {/* Completed banner */}
        {!updateStatus.running && updateStatus.completedAt && (
          <div className="fade" style={{ marginTop:10, fontSize:10, color:"#3ddc84" }}>
            ✓ Last full update completed {new Date(updateStatus.completedAt).toLocaleString()}
            {updateStatus.trigger === "nightly" ? " (nightly)" : " (manual)"}
          </div>
        )}
      ...
      <div style={{ display:"flex", height:"calc(100vh - 160px)" }}>
        ...
        <div style={{ flex:1, overflow:"auto", padding:"20px 24px" }}>
          ...
          {/* No data yet */}
          {doneCount === 0 && !updateStatus.running && (
            <div style={{ background:"#0d1120", border:"1px dashed #1e2540", borderRadius:6, padding:"30px", textAlign:"center", marginBottom:20 }}>
              <div style={{ fontSize:13, color:"#3a4560", marginBottom:12 }}>No data yet. Click <strong style={{ color:"#5baaff" }}>Update All Now</strong> to run the first scan.</div>
              <div style={{ fontSize:10, color:"#2a3050" }}>After that, updates run automatically every night at 2:00 AM UTC.</div>
            </div>
          )}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:10 }}>
            {filteredStates.map(s => {
              const d = stateData[s.code];
              const isUpdating = updateStatus.running && updateStatus.currentState === s.name;
              ...
              return (
                <div
                  ...
                  style={isUpdating ? { borderColor:"#5baaff", boxShadow:"0 0 12px #5baaff22" } : {}}
                >
        ...
        {selected && selState && (
          <div className="slide" style={{ width:380, background:"#0a0d1a", borderLeft:"1px solid #1e2540", overflow:"auto", padding:"24px 20px", flexShrink:0 }}>
            ...
            {!selData?.result && !updateStatus.running && (
              <div style={{ textAlign:"center", padding:"20px", fontSize:12, color:"#3a4560" }}>
                No data yet — click <strong style={{ color:"#5baaff" }}>Update All Now</strong> in the header to scan.
              </div>
            )}

            {updateStatus.running && updateStatus.currentState === selState.name && (
      ...
      <div style={{ background:"#0a0d1a", borderTop:"1px solid #1e2540", padding:"10px 28px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div style={{ fontSize:10, color:"#2a3050" }}>Sources: CMS.gov · 50 State DOH Websites · HHS.gov · Nightly 2:00 AM UTC</div>
