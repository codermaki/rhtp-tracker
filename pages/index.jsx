...
// ── Claude API (routed through server proxy to avoid CORS) ────────────────
async function callClaude(prompt, retries = 4) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    if (response.status === 429) {
      ...
      const wait = 20000 * (attempt + 1); // 20s, 40s, 60s, 80s
    ...
    if (!response.ok) {
      ...
      throw new Error(`Query failed: ${JSON.stringify(err)}`);
    }
...
export default function RHTPTracker() {
  ...
  const runUpdate = async () => {
    ...
    for (const state of STATES) {
      ...
      setProgress(done);
      // Pause between calls to stay under Anthropic rate limits
